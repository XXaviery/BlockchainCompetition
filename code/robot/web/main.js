...
const pollutionPanel = document.querySelector(".pollution-panel");
...
function applyMode(mode) {
  ...
  const labels = {
    ...
    pollution: "POLLUTION DEMO · READ ONLY",
    preview: "PREVIEW · NO CONTROL",
  };
  modeBadge.textContent = labels[serverMode] || labels.preview;
  modeBadge.dataset.mode = serverMode;
  const pollutionEnabled = serverMode === "pollution";
  setPanelDisabled(motionPanel, !motionEnabled());
  setPanelDisabled(bagPanel, !playbackEnabled());
  motionPanel.hidden = pollutionEnabled;
  bagPanel.hidden = pollutionEnabled;
  if (pollutionPanel) pollutionPanel.hidden = !pollutionEnabled;
  window.PollutionDemo?.setEnabled(pollutionEnabled);
...
async function refreshStatus() {
  try {
    ...
    } else if (payload.mode === "pollution") {
      setConnection("online", "Pollution demo · read only");
