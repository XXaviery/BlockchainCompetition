# MOF-Robot 双导航主线与实车验收记录

> 状态：执行基线  
> 建立日期：2026-08-13  
> 适用范围：底盘解算、IMU、EKF、SLAM/Nav2、轮速—惯性航位推算  
> 过程记录保存在本地部署记录中，不属于公开 Git 工作区。
...
模式 B 的工程名称应使用“轮速—惯性辅助航位推算”或“轮速—IMU 组合惯导”。纯 IMU 双积分只能作为后续实验，不作为正式导航输出。
...
- 编码器方向、电机方向、三轮运动学、轮径、轮距记录为固定底盘参数，不能因单次平移异常随意修改。
- `wheel_odom_angular_scale` 保持 `1.0`。
- 双向旋转使用冻结的 rotation PID 参数。
...
- 记录的静止 15 秒 EKF yaw 漂移约 `+0.217°`，记录中未见跳变。
...
上一 Gate 条件未满足时，不得跳到下一 Gate。

## 5. P0：非零运动前补强测试工具

电池充电期间仅处理电脑本地脚本、配置和文档并执行静态检查；电池充满前不连接树莓派、不发串口、不运动。
...
- 远端独有修改必须先备份、复制回电脑并合并，禁止直接覆盖本地未提交编辑内容。
- 所有持久修改只在电脑完成；本地检查完成后按文件白名单部署。
- 白名单文件双端 SHA-256 一致且远端构建命令返回码为 0 后，进入 Gate 0。

电脑端 `tools/ros_safe_roundtrip_test.py` 作为 P0 检查脚本；远端旧脚本不得覆盖电脑版本。
...
- 当前调试配置使用约 `0.3 s` 的 `velocity_smoother.velocity_timeout`。
- 外层超时管理优先发送 `SIGINT` 或 `SIGTERM`，不要直接强杀。

### 5.6 文档与版本留痕

- `/cmd_vel_smoothed` 不是旧 README、WASD 或测试工具的直接运动入口。
- `tools/chassis_tuner.py`、`tools/ros_rotation_test.py` 和 `tools/ros_safe_roundtrip_test.py` 已列入公开 Git 跟踪清单；本地部署记录不随源码发布。
- 部署记录包含参数、静态检查、部署哈希和回滚方式。
...
6. 真实障碍 blocked 场景使用新的 final 零速消息作为判据；旧脚本的判据
   与 `stop_pub_timeout` 语义不兼容，不用于 Nav2 主线。
...
每个 Gate 的本地部署记录描述时间、电源状态、部署前运行状态、执行命令、涉及文件及关键差异、本地与远端 SHA-256、ROS 节点与话题、TF、串口所有权、指令与执行器数据、编码器与 IMU 数据、wheel odom、EKF 数据、操作人员观察到的物理运动、运行结果、停止确认和回滚方法。该记录属于本地部署资料，不进入公开 Git 工作区。
...
电脑工作区是唯一持久源码真源，树莓派只是部署镜像和运行环境。禁止直接在树莓派项目源码中持续编辑。任何非零测试前先完成双端对账。
...
5. 当前电脑 `tools/ros_safe_roundtrip_test.py` 列入 P0 部署白名单；远端临时目录中的旧脚本不覆盖电脑端版本，也不直接用于非零运动。
6. 完成对账后，所有持久修改只在电脑进行。

先完成 P0：

1. 检查 tools/ros_safe_roundtrip_test.py：
   - 同时检查 /cmd_vel_nav、/cmd_vel_smoothed、/cmd_vel 的发布者和订阅者身份；
   - 检查 velocity_smoother 和 collision_monitor lifecycle 均为 active；
   - blocked 测试发出非零上游请求后，必须收到新的最终 /cmd_vel 零速消息，空消息记录为失败；
   - 普通折返必须看到非零 final、非零 ESP32 target 和最小有效位移，完全不动记录为失败；
   - MarkerArray 只统计 POINTS/ADD，检查 frame_id=base_footprint；
   - 捕获 SIGINT、SIGTERM、SIGHUP，统一连续发送零速；
   - 出程未产生有效运动、方向错误或任何异常时不得执行返程或第三段残差补偿。
   - 功能状态以 lifecycle 查询、5°偏航、0.02m横移、控制消息新鲜度、编码器最小变化量和停止信号的运行结果为依据。
   - wait_ready/run_leg/run_blocked_test/main 需要接入同一状态机后再进行验收。
2. 将 velocity_smoother 的 velocity_timeout 暂时收紧至约 0.3 秒。
3. 修正文档和旧工具中绕过安全链的 /cmd_vel_smoothed 默认入口。
4. 三份 tools 脚本和本地部署记录作为部署资料留存。
5. 部署记录包含哈希、静态检查和回滚信息。
...
5. 远端 colcon 构建命令返回码为 0，并确认 ROS 使用刚构建的 install 前缀。
...
- 操作人员观察真实物理方向，机器人周围至少留出 0.5 m；
- 偏航 >5°、横移 >0.02 m、单轮停转、PWM 持续饱和、遥测过期、拓扑变化或掉线时立即发零并停止；
- +Y 或对角小目标轮不转时，优先调查低速平移 mixed PID/启动补偿，不改运动学和方向逻辑。

Tier 1 条件全部满足后再做 0.08 m/1.2 秒，最后做 0.15 m/1.5 秒并每方向重复三次。

随后继续按 NAVIGATION_MAINLINE.md 的 Gate 3～Gate 5 完成平移参数收口、EKF+SLAM 建图和 EKF+SLAM/Nav2 的 A→B→A。

每个 Gate 条件记录数据表、物理观察、结论、停止状态和回滚方式，再进入下一 Gate。禁止跳过 Gate，禁止批量盲跑。
```

## 16. 运行职责

- 底盘解算与 EKF 运行检查覆盖 P0～Gate 5。
- 具备 Gate 5 的 EKF+SLAM/Nav2 `A -> B -> A` 冻结基线后，进入：

  ```text
  MOF-Robot 轮速惯导、相对返航、设备时间戳与 ESKF
  ```

- 只读检查可以覆盖配置、TF、话题、日志和算法。
- 所有机器人运动、SSH、串口写入和文件部署由单一控制端执行，禁止多个控制端同时操作底盘。

## 17. 运行基线摘要

- 电池耗尽掉线后的恢复入口为 Gate 0。
- 速度安全链、八方向分级折返和 EKF+SLAM/Nav2 运行条件按本文前述条目执行。
- 轮速惯导进入 ESKF 前需要具备设备时间戳、外参、标定和 ZUPT 条件。
