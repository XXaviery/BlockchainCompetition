...
- 代码：`src/zhiyu_brain/common/types.py`；`src/zhiyu_brain/simulation/environment.py`；`src/zhiyu_brain/sensing/quality.py`。
...
- 代码：`src/zhiyu_brain/state/region_state.py`。
...
- 代码：`src/zhiyu_brain/models/risk_model.py`。
...
- 代码：`src/zhiyu_brain/models/ranker_model.py`；`src/zhiyu_brain/decision/task_ranker.py`；`src/zhiyu_brain/decision/candidate.py`。
...
- 代码：`src/zhiyu_brain/task/state_machine.py`；`src/zhiyu_brain/task/manager.py`。
- 统一运行封装：`src/zhiyu_brain/runtime/software_loop.py`。
- 日志：`outputs/logs/brain.db` → `task_state_events`。
- 图片：`08_closed_loop_timeline.png`。

## 3.8 导航、避障与安全机制

- 导航接口：`src/zhiyu_brain/adapters/base.py`。
- Mock 导航：`src/zhiyu_brain/adapters/mock_navigation.py`。
- 安全监督：`src/zhiyu_brain/safety/supervisor.py`；`src/zhiyu_brain/task/manager.py::select_safe_task`。
- MODEL_ERROR fallback：`src/zhiyu_brain/runtime/software_loop.py` 调用 Phase-1 `RuleBasedPolicy`。
...
- 治理接口：`src/zhiyu_brain/adapters/base.py`。
- Mock 治理：`src/zhiyu_brain/adapters/mock_purification.py`。
- 环境响应：`src/zhiyu_brain/simulation/environment.py::set_purification`。
...
- Logger / DB：`src/zhiyu_brain/logging/store.py`。
...
- 规则策略：`src/zhiyu_brain/decision/rule_policy.py`。
- Ranker：`src/zhiyu_brain/models/ranker_model.py`。
