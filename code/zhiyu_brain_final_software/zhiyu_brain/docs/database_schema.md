...
最终 schema 只定义于 `src/zhiyu_brain/logging/store.py`，写入入口只使用 Phase-1 `BrainLogStore.log()`。Phase 1 原事件式日志与 Phase 2 typed schema 的冲突已合并：每张表同时保留 `event_key` / `payload_json` 追溯字段和 GUI/回放需要的 typed columns。
