...
- `core/`：ESP32 下位机固件，负责底盘运动控制、MS200 雷达解析和串口协议。
- `catkin_ws/src/mof_esp32_bridge/`：树莓派 ROS2 上位机包，负责串口桥接、发布 `/scan` 和 `/odom`，并提供 WASD、SLAM 建图和 Nav2 导航启动文件。
- `Web/`：本地 Web 控制台。

本目录 `code/code/` 是硬件、ROS2 和 Web 的唯一源码根目录。以下源码引用均相对于本目录；内层 `code/` 是重复仓库，仅保留在原工作区，不进入竞赛提交包。

## 源码路径与部署参数

源码路径固定使用相对形式：`core/`、`catkin_ws/src/mof_esp32_bridge/`、`Web/`、`tools/`。目标机工作空间、串口、地图、RViz 配置和 Web 端口由部署环境提供；它们不代表本源码树中的目录。例如：
...
从本源码根将 `catkin_ws/src/mof_esp32_bridge/` 复制到目标工作空间的 `src/`，然后在树莓派编译：
...
`/cmd_vel_nav`。详见 `catkin_ws/src/mof_esp32_bridge/NAVIGATION_MAINLINE.md`。
...
代码入口：[chassis.cpp](core/lib/chassis/chassis.cpp#L137-L149)
...
代码入口：[chassis.cpp](core/lib/chassis/chassis.cpp#L69-L71)
