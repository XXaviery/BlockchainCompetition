...
PROJECT_ROOT_ENV = 'AIR_GOVERNANCE_ROOT'
_ROOT_MARKERS = (
...
_ACTIVE_PROJECT_ROOT: ContextVar[Path | None] = ContextVar(
    'air_governance_active_project_root', default=None
)
...
def _validated_root(root: str | Path, label: str = 'air_governance root') -> Path:
    ...
    for anchor in (Path(__file__), Path.cwd()):
        for candidate in _candidate_roots(anchor):
            ...
            if _is_project_root(candidate):
                return candidate
    raise RuntimeError(
        f'Cannot locate the air_governance project root; set {PROJECT_ROOT_ENV}.'
    )
